    ...
    <>
      ...
      <h1 className="text-3xl md:text-4xl font-bold text-white mb-10 sm:mb-20">
        ...
        <div className='w-full flex gap-5'>
          ...
          {
            cardItems.map((item) => (
<div key={item.id} className='group bg-slate-500 overflow-hidden rounded-xl relative w-[200px] h-[200px] z-50'>
  {/* Image */}
  <img
    className='w-full p-1 h-full rounded-xl object-cover z-50'
    src={item.logo}
    alt=""
  />
          {/* 🔹 Background Overlay 1 - Gradient Fade at Bottom */}
          <div className='absolute bottom-10 inset-x-0 h-full bg-red-600 z-5'></div>
...
{/* 🔹 Background Overlay 1 - Gradient Fade at Bottom */}
  ...
  {/* 🔸 Background Overlay 2 - Full hover overlay */}
  <div className='absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-1000 ease-in-out bg-black/40 z-20'></div>
  {/* Top Label Tag */}
